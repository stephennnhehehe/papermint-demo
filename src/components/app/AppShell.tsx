...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen max-w-full overflow-x-clip px-4 py-4 sm:px-6 lg:px-8">
      <header className="relative z-50 mx-auto flex w-full min-w-0 max-w-[1600px] flex-col gap-3 rounded-lg border border-[var(--line)] bg-white/80 p-3 shadow-sm backdrop-blur lg:flex-row lg:items-center lg:justify-between">
        <Link className="flex min-w-0 items-center gap-3" href="/dashboard">
          <Brand />
          <span className="min-w-0 max-w-48 flex-1 border-l border-[var(--line)] pl-3">
            <span className="block text-[10px] font-semibold leading-4 text-[var(--muted)] sm:text-xs">
              {t("appTagline")}
            </span>
          </span>
        </Link>

        <nav className="grid w-full min-w-0 grid-cols-2 gap-1 rounded-lg border border-[var(--line)] bg-[#f8faf7] p-1 sm:grid-cols-3 lg:flex lg:w-auto lg:flex-nowrap">
          {nav.map((item) => {
            const Icon = item.icon;
            const active = pathname.startsWith(item.href);
            return (
              <Link
                className={`inline-flex min-w-0 items-center justify-center gap-1.5 rounded-md px-2.5 py-2 text-xs font-bold sm:text-sm ${
