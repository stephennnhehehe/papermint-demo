...
import { tripKilometres, vehicleLogbookSummary } from "@/lib/vehicle-logbook";
...
type TripForm = {
  ...
  is_business: boolean; driver: string; notes: string;
};
...
const tripForm = (): TripForm => ({
  ...
  purpose: "", start_odometer: "", end_odometer: "", is_business: true, driver: "", notes: ""
});
  ...
  async function handleTripSubmit(event: FormEvent) {
    ...
    try {
      await upsertVehicleTrip(user.id, {
        ...
        is_business: trip.is_business, driver: trip.driver.trim() || null, notes: trip.notes.trim() || null
      });
    ...
    <ProtectedRoute><AppShell>
      ...
      {tab === "logbook" ? <div className="grid gap-5">
        <div className="grid gap-5 xl:grid-cols-2">
          ...
          <section className="panel p-5"><SectionTitle icon={<Route className="h-5 w-5" />} title={trip.id ? copy({ en: "Edit journey", zh: "编辑行程" }) : copy({ en: "Add logbook journey", zh: "添加 Logbook 行程" })} subtitle={copy({ en: "Record each business and private trip so the business-use percentage has a complete denominator.", zh: "同时记录业务与私人行程，才能得到完整的业务使用比例。" })} /><form className="mt-5 grid gap-3" onSubmit={handleTripSubmit}><Select label={copy({ en: "Vehicle", zh: "车辆" })} value={trip.vehicle_id} onChange={(value) => setTrip({ ...trip, vehicle_id: value })} options={vehicleOptions.slice(1)} /><div className="grid grid-cols-2 gap-3"><Field label={copy({ en: "Start date", zh: "开始日期" })} type="date" value={trip.start_date} onChange={(value) => setTrip({ ...trip, start_date: value, end_date: trip.end_date < value ? value : trip.end_date })} /><Field label={copy({ en: "End date", zh: "结束日期" })} type="date" value={trip.end_date} onChange={(value) => setTrip({ ...trip, end_date: value })} /></div><div className="grid grid-cols-2 gap-3"><Field label={copy({ en: "From", zh: "出发地" })} required value={trip.origin} onChange={(value) => setTrip({ ...trip, origin: value })} /><Field label={copy({ en: "To", zh: "目的地" })} required value={trip.destination} onChange={(value) => setTrip({ ...trip, destination: value })} /></div><Field label={copy({ en: "Purpose of journey", zh: "行程目的" })} required value={trip.purpose} onChange={(value) => setTrip({ ...trip, purpose: value })} /><div className="grid grid-cols-2 gap-3"><Field inputMode="decimal" label={copy({ en: "Start odometer", zh: "开始里程" })} required value={trip.start_odometer} onChange={(value) => setTrip({ ...trip, start_odometer: value })} /><Field inputMode="decimal" label={copy({ en: "End odometer", zh: "结束里程" })} required value={trip.end_odometer} onChange={(value) => setTrip({ ...trip, end_odometer: value })} /></div><p className="text-xs font-black text-[var(--mint-dark)]">{copy({ en: "Distance", zh: "里程" })}: {Math.max(0, Number(trip.end_odometer) - Number(trip.start_odometer)).toFixed(1)} km</p><Select label={copy({ en: "Journey type", zh: "行程类型" })} value={trip.is_business ? "business" : "private"} onChange={(value) => setTrip({ ...trip, is_business: value === "business" })} options={[{ value: "business", label: copy({ en: "Business", zh: "业务" }) }, { value: "private", label: copy({ en: "Private", zh: "私人" }) }]} /><Field label={copy({ en: "Driver (optional)", zh: "司机（可选）" })} value={trip.driver} onChange={(value) => setTrip({ ...trip, driver: value })} /><div className="flex gap-2"><button className="btn-primary flex-1" disabled={saving || vehicles.length === 0} type="submit"><Plus className="h-4 w-4" />{t("save")}</button>{trip.id ? <button className="btn-secondary" onClick={() => setTrip(tripForm())} type="button">{copy({ en: "Cancel", zh: "取消" })}</button> : null}</div></form></section>
        </div>
        ...
        <section className="panel min-w-0 p-5"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-black">{copy({ en: "Journey register", zh: "行程记录" })}</h2><p className="text-sm font-semibold text-[var(--muted)]">{visibleTrips.length} {copy({ en: "journeys", zh: "条行程" })} · {visibleTrips.reduce((sum, item) => sum + tripKilometres(item), 0).toFixed(1)} km</p></div><Select label={copy({ en: "Vehicle filter", zh: "车辆筛选" })} value={vehicleFilter} onChange={setVehicleFilter} options={[{ value: "all", label: copy({ en: "All vehicles", zh: "全部车辆" }) }, ...vehicles.map((item) => ({ value: item.id, label: item.name }))]} /></div>{visibleTrips.length === 0 ? <Empty>{copy({ en: "No journeys in this view.", zh: "当前没有行程记录。" })}</Empty> : <div className="overflow-x-auto"><table className="w-full min-w-[960px] text-left text-sm"><thead><tr className="border-b border-[var(--line)] text-xs uppercase text-[var(--muted)]"><th className="py-3">{copy({ en: "Date / Vehicle", zh: "日期 / 车辆" })}</th><th>{copy({ en: "Route", zh: "路线" })}</th><th>{copy({ en: "Purpose", zh: "目的" })}</th><th>{copy({ en: "Type", zh: "类型" })}</th><th className="text-right">km</th><th /></tr></thead><tbody>{visibleTrips.map((item) => <tr className="border-b border-[#eef2ef]" key={item.id}><td className="py-3"><span className="font-black">{item.start_date}{item.end_date !== item.start_date ? ` – ${item.end_date}` : ""}</span><span className="block text-xs text-[var(--muted)]">{vehicles.find((v) => v.id === item.vehicle_id)?.name ?? "-"}</span></td><td>{item.origin} → {item.destination}<span className="block text-xs text-[var(--muted)]">{item.start_odometer} → {item.end_odometer}</span></td><td>{item.purpose}</td><td><span className={`rounded-full px-2 py-1 text-xs font-black ${item.is_business ? "bg-[#e8f4ef] text-[var(--mint-dark)]" : "bg-[#f1f3f1] text-[var(--muted)]"}`}>{item.is_business ? copy({ en: "Business", zh: "业务" }) : copy({ en: "Private", zh: "私人" })}</span></td><td className="text-right font-black">{tripKilometres(item)}</td><td><div className="flex justify-end gap-1"><button className="icon-btn" onClick={() => setTrip({ id: item.id, vehicle_id: item.vehicle_id, start_date: item.start_date, end_date: item.end_date, origin: item.origin, destination: item.destination, purpose: item.purpose, start_odometer: String(item.start_odometer), end_odometer: String(item.end_odometer), is_business: item.is_business, driver: item.driver ?? "", notes: item.notes ?? "" })} type="button"><Pencil className="h-4 w-4" /></button><button className="icon-btn text-[var(--rose)]" onClick={() => removeTrip(item)} type="button"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}</tbody></table></div>}</section>
      </div> : null}
