...
import { australianFiscalYear, calculateBasSummary, inDateRange, rowsToCsv } from "@/lib/financials";
...
const categories: ExpenseCategory[] = ["materials", "fuel", "software", "phone", "marketing", "professional_services", "travel", "office", "other"];
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[390px_minmax(0,1fr)]">
        <section className="panel min-w-0 p-5">
          ...
          <form className="grid gap-3" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-2 gap-3"><Field label={copy({ en: "Date", zh: "日期", vi: "Ngày", ar: "التاريخ" })} type="date" value={form.expense_date} onChange={(value) => setForm({ ...form, expense_date: value })} /><Select label={copy({ en: "Category", zh: "类别", vi: "Danh mục", ar: "الفئة" })} value={form.category} onChange={(value) => setForm({ ...form, category: value as ExpenseCategory })} options={categories.map((category) => ({ value: category, label: category.replaceAll("_", " ") }))} /></div>
            ...
            <Select label={copy({ en: "Purchase type", zh: "采购类型", vi: "Loại mua hàng", ar: "نوع الشراء" })} value={form.purchase_type} onChange={(value) => setForm({ ...form, purchase_type: value as ExpenseForm["purchase_type"] })} options={[{ value: "non_capital", label: "Non-capital (G11)" }, { value: "capital", label: "Capital (G10)" }]} />
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: Array<{ value: string; label: string }> }) {
  return <label className="min-w-44"><span className="label">{label}</span><select className="field" value={value} onChange={(event) => onChange(event.target.value)}>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>;
}
